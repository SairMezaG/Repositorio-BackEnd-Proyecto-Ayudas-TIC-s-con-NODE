...
const storage = multer.diskStorage({
    ...
    filename: function (req, file, cb) {
        ...
        // Crear un nombre único usando la fecha actual
        const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
        
        // Formar el nombre del archivo con el correo sanitizado y un sufijo único
        const filename = `${userEmail}-${uniqueSuffix}${extFile}`;
